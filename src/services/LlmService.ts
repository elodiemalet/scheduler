...
/**
 * Client construit à la première génération, pas au chargement du module :
 * sans cela, une clé absente casserait aussi la lecture du planning existant,
 * qui n'appelle pourtant jamais le modèle.
 */
...
function getClient(): OpenAI {
    if (!client) {
        const env = getEnv();
        client = new OpenAI({apiKey: env.llmApiKey, baseURL: env.llmBaseUrl});
    }
    return client;
}
